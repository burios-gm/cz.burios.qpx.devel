...
/** SQL-like fluent facade over the QL SELECT AST. */
public final class DSLSelect {
    ...
    public static final class Builder {
        ...
        public Builder column(String name) { return column(col(name)); }
        public Builder columns(QLExpr... expressions) { if (expressions != null) select.columns.addAll(Arrays.asList(expressions)); return this; }
        ...
        public <T extends BasicRecord> T one(Connection connection, Class<T> type) throws SQLException { List<T> rows = execute(connection, type); if (rows.isEmpty()) return null; if (rows.size() > 1) throw new SQLException("Expected one row, got " + rows.size()); return rows.get(0); }
