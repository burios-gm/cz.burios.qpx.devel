...
/** H2 dialect. H2 commits DDL implicitly, so transactional schema migration is not reliable. */
public class H2Dialect implements DBDialect {
    ...
    private String type(ColumnMetaData c) {
        if (c.logicalType != null) return switch (c.logicalType) {
            ...
            case TEXT -> "CLOB"; case BOOLEAN -> "BOOLEAN"; case INTEGER -> "INTEGER"; case LONG -> "BIGINT";
            case DECIMAL -> c.precision > 0 ? "DECIMAL(" + c.precision + "," + Math.max(c.scale, 0) + ")" : "DECIMAL";
            case DOUBLE -> "DOUBLE"; case DATE -> "DATE"; case TIME -> "TIME"; case DATETIME, TIMESTAMP -> "TIMESTAMP";
