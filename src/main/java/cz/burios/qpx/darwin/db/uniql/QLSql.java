...
public final class QLSql implements QLVisitor {
    ...
    public static Result render(QLSelect select) {
        ...
        QLSql v = new QLSql(); select.accept(v);
    ...
    @Override public void visit(QLSelect e) {
        ...
        for(QLJoin j:e.joins){sql.append(' ');expr(j);}
        ...
        if(e.having!=null){sql.append(" HAVING ");expr(e.having);}
    ...
    @Override public void visit(QLCase e){sql.append("CASE");if(e.operand!=null){sql.append(' ');expr(e.operand);}for(QLCase.When w:e.whens){sql.append(" WHEN ");expr(w.condition);sql.append(" THEN ");expr(w.result);}if(e.otherwise!=null){sql.append(" ELSE ");expr(e.otherwise);}sql.append(" END");alias(e.alias);if(e.alias!=null&&!e.alias.isBlank())sql.append(" AS ").append(e.alias);}
