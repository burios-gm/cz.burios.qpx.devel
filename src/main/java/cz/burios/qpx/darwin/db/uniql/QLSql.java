...
public final class QLSql implements QLVisitor {
    ...
    @Override public void visit(QLCondition e){sql.append('(');expr(e.left);sql.append(' ').append(e.operator).append(' ');expr(e.right);sql.append(')');}
    @Override public void visit(QLLogical e){String x=e.operator==null?"":e.operator.trim().toUpperCase();if(x.equals("NOT")){if(e.expressions.size()!=1)throw new IllegalArgumentException("NOT requires one expression");sql.append("(NOT ");expr(e.expressions.get(0));sql.append(')');return;}if(!x.equals("AND")&&!x.equals("OR"))throw new IllegalArgumentException("Logical operator must be AND, OR or NOT");sql.append('(');for(int i=0;i<e.expressions.size();i++){if(i>0)sql.append(' ').append(x).append(' ');expr(e.expressions.get(i));}sql.append(')');}
    @Override public void visit(QLValue e){sql.append('?');parameters.add(e.value);}
    @Override public void visit(QLFunction e){identifier(e.name,"function");sql.append(e.name).append('(');if(e.distinct)sql.append("DISTINCT ");for(int i=0;i<e.arguments.size();i++){if(i>0)sql.append(", ");expr(e.arguments.get(i));}sql.append(')');alias(e.alias);if(e.alias!=null&&!e.alias.isBlank())sql.append(" AS ").append(e.alias);}
    ...
    public void visit(QLExists e){sql.append('(').append(e.negated?"NOT EXISTS ":"EXISTS ");expr(e.subSelect);sql.append(')');}
    public void visit(QLCase e){sql.append("CASE");if(e.operand!=null){sql.append(' ');expr(e.operand);}for(QLCase.When w:e.whens){sql.append(" WHEN ");expr(w.condition);sql.append(" THEN ");expr(w.result);}if(e.otherwise!=null){sql.append(" ELSE ");expr(e.otherwise);}sql.append(" END");alias(e.alias);if(e.alias!=null&&!e.alias.isBlank())sql.append(" AS ").append(e.alias);}
