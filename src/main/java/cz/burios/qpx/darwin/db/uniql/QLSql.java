...
public final class QLSql implements QLVisitor {
    ...
    private static void identifier(String value, String kind) {
        if (value == null || !IDENTIFIER.matcher(value).matches()) throw new IllegalArgumentException("Invalid " + kind + ": " + value);
    }
    private static void alias(String value) {
        if (value != null && !ALIAS.matcher(value).matches()) throw new IllegalArgumentException("Invalid SQL alias: " + value);
    }
    ...
    @Override public void visit(QLSelect e) {
        ...
        else for (int i = 0; i < e.columns.size(); i++) { if (i > 0) sql.append(", "); expr(e.columns.get(i)); }
        if (e.from == null) throw new IllegalStateException("SELECT requires FROM");
        sql.append(" FROM "); expr(e.from);
    ...
    @Override public void visit(QLColumn e) {
        identifier(e.name, "column"); sql.append(e.name); alias(e.alias);
        if (e.alias != null && !e.alias.isBlank()) sql.append(" AS ").append(e.alias);
    }
    @Override public void visit(QLTable e) {
        identifier(e.name, "table"); sql.append(e.name); alias(e.alias);
        if (e.alias != null && !e.alias.isBlank()) sql.append(" AS ").append(e.alias);
    }
    @Override public void visit(QLJoin e) {
        String type = e.type == null ? "INNER" : e.type.trim().toUpperCase();
        if (!switch (type) { case "INNER", "LEFT", "RIGHT", "FULL", "CROSS" -> true; default -> false; }) throw new IllegalArgumentException("Unsupported JOIN type: " + e.type);
        sql.append(type).append(" JOIN "); expr(e.table);
        if (e.on != null) { sql.append(" ON "); expr(e.on); }
    }
    @Override public void visit(QLWhere e) {
        for (int i = 0; i < e.expressions.size(); i++) { if (i > 0) sql.append(" AND "); sql.append('('); expr(e.expressions.get(i)); sql.append(')'); }
    }
    @Override public void visit(QLCondition e) {
        sql.append('('); expr(e.left); sql.append(' ').append(operator(e.operator)).append(' '); expr(e.right); sql.append(')');
    }
    @Override public void visit(QLValue e) { sql.append('?'); parameters.add(e.value); }
    @Override public void visit(QLFunction e) {
        identifier(e.name, "function"); sql.append(e.name).append('(');
        for (int i = 0; i < e.arguments.size(); i++) { if (i > 0) sql.append(", "); expr(e.arguments.get(i)); }
        sql.append(')'); alias(e.alias); if (e.alias != null && !e.alias.isBlank()) sql.append(" AS ").append(e.alias);
    }
    @Override public void visit(QLSubSelect e) {
        if (e.select == null) throw new IllegalStateException("Subselect requires SELECT");
        sql.append('('); e.select.accept(this); sql.append(')'); alias(e.alias);
        if (e.alias != null && !e.alias.isBlank()) sql.append(" AS ").append(e.alias);
    }
    @Override public void visit(QLGroupBy e) {
        for (int i = 0; i < e.expressions.size(); i++) { if (i > 0) sql.append(", "); expr(e.expressions.get(i)); }
    }
    @Override public void visit(QLOrderBy e) {
        for (int i = 0; i < e.items.size(); i++) {
            if (i > 0) sql.append(", "); expr(e.items.get(i).expression);
