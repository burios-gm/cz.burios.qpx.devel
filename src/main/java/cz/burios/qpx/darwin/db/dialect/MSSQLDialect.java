...
public class MSSQLDialect implements DBDialect {
    ...
    @Override public String tableName(TableMetaData table) {
        ...
        if (table.database != null && !table.database.isBlank()) sql.append(quote(table.database)).append('.');
        if (table.schema != null && !table.schema.isBlank()) sql.append(quote(table.schema)).append('.');
        return sql.append(quote(table.name)).toString();
    }
    ...
    @Override public ColumnType logicalType(ColumnMetaData c) {
        ...
        if (nativeType != null) {
            ...
            if (t.contains("TEXT") || t.contains("NTEXT")) return ColumnType.TEXT;
            if (t.equals("BIT")) return ColumnType.BOOLEAN;
            if (t.equals("BIGINT")) return ColumnType.LONG;
            if (t.equals("INT") || t.equals("INTEGER") || t.equals("SMALLINT") || t.equals("TINYINT")) return ColumnType.INTEGER;
            if (t.contains("DECIMAL") || t.contains("NUMERIC") || t.equals("MONEY") || t.equals("SMALLMONEY")) return ColumnType.DECIMAL;
            if (t.contains("FLOAT") || t.equals("REAL")) return ColumnType.DOUBLE;
            if (t.equals("DATETIME") || t.equals("DATETIME2") || t.equals("SMALLDATETIME")) return ColumnType.DATETIME;
            if (t.equals("DATETIMEOFFSET")) return ColumnType.TIMESTAMP;
            if (t.equals("DATE")) return ColumnType.DATE;
            if (t.equals("TIME")) return ColumnType.TIME;
            if (t.contains("BINARY") || t.contains("IMAGE")) return ColumnType.BINARY;
        }
    ...
    @Override public String columnDefinition(ColumnMetaData c) {
        ...
        if (c.autoIncrement) sql.append(" IDENTITY(1,1)");
    ...
    @Override public String columnGeneration(ColumnMetaData c) {
        return switch (c.generation == null ? ColumnGeneration.NONE : c.generation) {
            case NONE -> "";
            case INSERT_TIMESTAMP, INSERT_UPDATE_TIMESTAMP -> "DEFAULT CURRENT_TIMESTAMP";
        };
    }
    ...
    @Override public String createIndex(TableMetaData table, IndexMetaData index) {
        return DBDialect.super.createIndex(table, index);
    }

    @Override public String dropIndex(TableMetaData table, String name) {
        return "DROP INDEX " + indexName(name) + " ON " + tableName(table);
    }
    ...
    private String type(ColumnMetaData c) {
        if (c.logicalType != null) return switch (c.logicalType) {
            ...
            case INTEGER -> "INT";
            case LONG -> "BIGINT";
            case DECIMAL -> c.precision > 0 ? "DECIMAL(" + c.precision + "," + Math.max(c.scale, 0) + ")" : "DECIMAL(18,2)";
            case DOUBLE -> "FLOAT";
            case DATE -> "DATE";
            case TIME -> "TIME";
            case DATETIME -> "DATETIME2";
            case TIMESTAMP -> "DATETIME2";
            case BINARY -> c.length > 0 ? "VARBINARY(" + c.length + ")" : "VARBINARY(MAX)";
        };
        if (c.type != null && !c.type.isBlank() && c.type.matches("[A-Za-z][A-Za-z0-9_]*(\\s*\\(\\s*[0-9]+(?:\\s*,\\s*[0-9]+)?\\s*\\))?")) return c.type.trim();
        return switch (c.jdbcType) {
            ...
            case Types.BOOLEAN, Types.BIT -> "BIT";
            case Types.DATE -> "DATE";
            case Types.TIME -> "TIME";
            case Types.TIMESTAMP -> "DATETIME2";
            case Types.BINARY, Types.VARBINARY, Types.LONGVARBINARY -> "VARBINARY(MAX)";
