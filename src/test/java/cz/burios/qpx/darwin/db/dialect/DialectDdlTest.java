...
public class DialectDdlTest {
    ...
    private static TableMetaData table() {
        return new TableMetaData("STORE").database("depo_cz").schema("dbo");
    }

    private static ColumnMetaData name() {
        return new ColumnMetaData("NAME").string(80).nullable(false);
    }

    private static IndexMetaData index() {
        return new IndexMetaData("IX_STORE_NAME").unique(true).method("BTREE").column("NAME").column("ID");
    }

    private static void testMySql() {
        MySQLDialect d = new MySQLDialect();
        TableMetaData t = table();
    ...
    private static void testMssql() {
        ...
        if (!"[ID] BIGINT IDENTITY(1,1) NOT NULL PRIMARY KEY".equals(d.columnDefinition(id))) throw new AssertionError("MSSQL PK: " + d.columnDefinition(id));
